...
@section('title', 'Edit Aturan Diskon')
@section('page_title', 'Edit Aturan Diskon')

@section('content')
<div class="max-w-2xl">
    <div class="bg-surface rounded-xl border border-border-soft shadow-sm p-6">
        <form action="{{ route('aturan-diskon.update', $aturanDiskon->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="produk_id" required
                    ...
                    <option value="{{ $produk->id }}" @selected(old('produk_id', $aturanDiskon->produk_id) == $produk->id)>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="tipe_diskon" id="tipeDiskon" required
                        class="w-full px-4 py-2.5 border border-border-soft rounded-xl focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition">
                        <option value="nominal" @selected(old('tipe_diskon', $aturanDiskon->tipe_diskon) == 'nominal')>Nominal (Rp)</option>
                        <option value="persen" @selected(old('tipe_diskon', $aturanDiskon->tipe_diskon) == 'persen')>Persen (%)</option>
                        <option value="free-packaging" @selected(old('tipe_diskon', $aturanDiskon->tipe_diskon) == 'free-packaging')>Gratis Pengemasan (Rp/unit)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="nilai_diskon" value="{{ old('nilai_diskon', $aturanDiskon->nilai_diskon) }}"
                        class="w-full px-4 py-2.5 border border-border-soft rounded-xl focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition"
                        required>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $aturanDiskon->tanggal_mulai?->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $aturanDiskon->tanggal_selesai?->format('Y-m-d')) }}"
                        class="w-full px-4 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition">
            ...
            <label class="flex items-center space-x-3 pt-1">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $aturanDiskon->is_active))
